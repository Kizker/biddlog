        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button 
              ...
              style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--line)', background: filter === 'hari_ini' ? 'var(--navy)' : 'white', color: filter === 'hari_ini' ? 'white' : 'var(--text)', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
              Hari Ini (Real-time)
            </button>
            <button 
              onClick={() => setFilter('minggu_ini')}
              style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--line)', background: filter === 'minggu_ini' ? 'var(--navy)' : 'white', color: filter === 'minggu_ini' ? 'white' : 'var(--text)', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
              Minggu Ini
            </button>
            <button 
              onClick={() => setFilter('bulan_ini')}
              style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--line)', background: filter === 'bulan_ini' ? 'var(--navy)' : 'white', color: filter === 'bulan_ini' ? 'white' : 'var(--text)', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
              Bulan Ini
            </button>
          </div>
        </div>
