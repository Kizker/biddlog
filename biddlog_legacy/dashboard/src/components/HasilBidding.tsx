    ...
    <div style={{ padding: '24px', background: 'white', borderRadius: '12px', border: '1px solid var(--line)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <p style={{ color: '#64748b', fontSize: '13px' }}>Modul ini mencocokkan target (barang yang dibagikan) dengan list aktual yang didapatkan anggota. Jika selisih harga melebihi batas, Admin bisa memberikan validasi (ACC) di sini.</p>
                  ...
                  return (
                    <tr key={obt.id} style={{ borderBottom: '1px solid var(--line)' }}>
                      ...
                      <td style={{ padding: '12px' }}>
                        {overlimit ? (
                          <span style={{ background: '#fee2e2', color: '#b91c1c', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>Lewat Limit</span>
                        ) : target ? (
                          <span style={{ background: '#dcfce3', color: '#166534', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>Sesuai (Cair)</span>
                        ) : (
                          <span style={{ background: '#f1f5f9', color: '#475569', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>Tanpa Target</span>
                        )}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'center' }}>
                        {overlimit && (
                          <button style={{ padding: '6px 12px', background: 'var(--navy)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>ACC Gaji</button>
                        )}
                      </td>
                    </tr>
                  )
